...
class HomeArticleSection extends StatelessWidget {
    ...
    return BlocBuilder<HomeArticleBannerBloc, HomeArticleBannerState>(
      builder: (context, state) {
        if (state is HomeArticleBannerSuccess) {
          ...
          return _buildSuccess(context, datas);
        }
  ...
  Widget _buildSuccess(BuildContext context, List<ArticleModel> listData) {
      ...
      children: [
        Container(
          ...
          child: HomeTitleDivider(
            title: LocaleKeys.home_section_article_title.tr(),
            onTap: () {
              context.pushRoute(const ArticleRouter());
            },
          ),
        ),
        const SizedBox(height: 5),
                ...
                return InkWell(
                  onTap: () {
                    if (data.actionType == ArticleActionType.screen) {
                      getIt.get<AppRouter>().navigateNamed(data.path!,
